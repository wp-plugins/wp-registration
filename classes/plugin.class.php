	...
	*/
	function create_user() {
		...
		if ( $userid && !is_wp_error( $userid ) ) {
			...
			/*foreach( $submitted_data as $meta_key => $meta_value ){
				update_user_meta( $userid, $meta_key, $meta_value );	
			}
			
			$code = sha1( $userid . time() );
			$activation_link = '<a href="'.add_query_arg( array( 'key' => $code, 'user' => $userid ), get_permalink( 176 )).'"> CLICK HERE TO COMPLETE YOUR REGISTRATION! </a>';
			add_user_meta( $userid, 'has_to_be_activated', $code, true );
			$headers [] = "From: Horizons Estate Agents <wordpress@horizons-estateagents.co.uk>";
			// $headers [] = "Reply-To: $reply_to";
			$headers [] = "Content-Type: text/html";
			
			$msg = $this -> get_option('_email_activate');
			
			
			$msg = str_replace("%USERNAME%", $nm_email, $msg);
			...
			//$msg .= ' User login: '.$nm_email.' User password: '.$pwd . ' CLICK HERE TO COMPLETE YOUR REGISTRATION! ' . $activation_link;
			$msg .= '<br/><img src="http://www.horizons-estateagents.co.uk/wp-content/uploads/2014/09/Horizons-Estate-Agents-Logo-80.png">';
			$subject = $this -> get_option('_new_user_subject');*/
			//$headers [] = "Content-Type: text/html";
			//$msg = 'Username: '.$wp_registration_login;
			//$msg .= '<p>Password: '.$pwd;
			...
			//$msg = str_replace("%USER_LOGIN%", $nm_email, $msg);
			$msg = str_replace("%USER_PASSWORD%", $pwd, $msg);
			$msg = str_replace("%SITE_NAME%", $team, $msg);
			$msg .= '<p><a href="' . wp_login_url() . '" title="Login">Click here to login</a>';
			$msg = str_replace("\n", "<br />", $msg);

			wp_mail( $wp_registration_email, 'registration', $msg, $headers);
			...
			/*$redirect_now = $this -> get_option('_redirect_url');
			//$redirect_now = ($redirect_now == '' ? wp_login_url() : $redirect_now);
			if ($redirect_now != ''){
				wp_redirect( $redirect_now );
				exit;
			}*/
		} else {
		...
		/*if ($userid){
			$msg = 'User login: '.$nm_email.' User password: '.$pwd;
			wp_mail($nm_email, 'Login Info', $msg);
		}*/
		/*print_r($_REQUEST);
		echo $userid;*/
		die(0);
